...
namespace OneDrag
{
    class ProcessClass
    {
        ...
        private void CodingStartVocal(object Path)
        {
            ...
            CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -i \"" + Path + "\" -vn -sn -af \"volumedetect\" -f null null";
            OutputReceived("Command - " + CMD);
            CreateProcess("cmd.exe", "/c " + CMD);  //info

            double? volume = 0 - max_volume - 6;

            CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -af \"volume=" + volume + "dB\" -c:a pcm_s16le -f wav -map_channel 0.0.0 \"" + Folder + "OneDrag_" + FileName + "_L.wav\" -map_channel 0.0.1 \"" + Folder + "OneDrag_" + FileName + "_R.wav\"";
            OutputReceived("Command - " + CMD);
            CreateProcess("cmd.exe", "/c " + CMD);  //coding

            if (video)
            {
                File.Delete(Folder + "OneDrag_" + FileName + "_L.wav");
                CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -af \"volume=" + volume + "dB\" -c:a pcm_s16le -f wav -map_channel 0.1.0 \"" + Folder + "OneDrag_" + FileName + "_L.wav\" -map_channel 0.1.1 \"" + Folder + "OneDrag_" + FileName + "_R.wav\""; OutputReceived("Command - " + CMD);
                CreateProcess("cmd.exe", "/c " + CMD);  //coding
                if (invalid)
                {
                    File.Delete(Folder + "OneDrag_" + FileName + "_L.wav");
                    CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -af \"volume=" + volume + "dB\" -c:a pcm_s16le -f wav -map_channel 0.1.0 \"" + Folder + "OneDrag_" + FileName + "_M.wav\"";
                    OutputReceived("Command - " + CMD);
                    CreateProcess("cmd.exe", "/c " + CMD);  //coding
                }
            }
            ...
            {
                File.Delete(Folder + "OneDrag_" + FileName + "_L.wav");
                CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -af \"volume=" + volume + "dB\" -c:a pcm_s16le -f wav -map_channel 0.0.0 \"" + Folder + "OneDrag_" + FileName + "_M.wav\"";
                OutputReceived("Command - " + CMD);
                CreateProcess("cmd.exe", "/c " + CMD);  //coding
            }
        }
        ...
        private void CodingStartMusic(object Path)
        {
            ...
            CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -af \"volume=" + volume + "dB\" -c:a pcm_s16le -f wav \"" + Folder + "OneDrag_" + FileName + "_S.wav\"";
            OutputReceived("Command - " + CMD);
            CreateProcess("cmd.exe", "/c " + CMD);  //coding
        ...
        private void CodingStartMixed(object Path)
        {
            ...
            if(FM.ToUpper() == "MP3")
            {
                CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -c:a pcm_s16le -f wav pipe: | .\\bin\\lame -b " + CR + " -p - \"" + Folder + "OneDrag_" + FileName + "." + FM + "\"";
            }
            else
            {
                CMD = "cd \"" + Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "\\OneDrag\" && .\\Bin\\ffmpeg -y -i \"" + Path + "\" -vn -sn -ar " + SR + " -c:a pcm_s16le -f wav pipe: | .\\bin\\neroAacEnc -ignorelength -cbr " + CR * 1000 + " -if - -of \"" + Folder + "OneDrag_" + FileName + "." + FM + "\"";
            }
            OutputReceived("Command - " + CMD);
            CreateProcess("cmd.exe", "/c " + CMD);  //coding
